...
`IMPORT TEXTURE...` adds a new YTD to an existing custom component. Select a
custom result/YDD, or enter its in-game clothing number, then choose the source
YTD. The tool assigns the next texture letter, renames the internal texture,
updates the drawable's YMT texture data, and backs up the YMT first.
...
- The locator itself makes no network requests and never asks for administrator rights or Defender exclusions.
